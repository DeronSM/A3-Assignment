...
public class University
{
    ...
    {
        //TODO done
        Boolean currentlyVisiting = beingVisited.get(node);
        //to check if key is present and its value is true
        if (currentlyVisiting != null && currentlyVisiting)
        {
            return true;
        }
        Boolean hasBeenVisited = visited.get(node);
        //to check if key is present and its value is true
        ...
        for (Module neighbor : node.getPrerequisites())
        {
        ...
        // to remove it from the recursion
    ...
    protected Module searchForModule(ArrayList<Cohort> list, String moduleName)
    {
        ...
        {
            int mid = lb + (ub - lb) / 2;
            String currentModuleName = list.get(mid).getModule().getName();
            int cmp = currentModuleName.compareTo(moduleName);
            if (cmp == 0)
            {
                return list.get(mid).getModule();
            }
            else if (cmp < 0)
            {
                lb = mid + 1;
    ...
    protected ArrayList<Cohort> sortMethod(ArrayList<Cohort> list, int block_size, boolean ascending, String attr) {
        ...
        for (int i = 1; i < n; i++)
        {
            Cohort key = list.get(i);
            int j = i - 1;

            while (j >= 0)
            {
                Cohort current = list.get(j);
                int comparison;

                if (attr.equalsIgnoreCase("name"))
                {
                    comparison = current.getModule().getName().compareTo(key.getModule().getName());
                }
                else
                {
                    comparison = current.getModule().getCode() - key.getModule().getCode();
                }
