...
public class BinaryTree
{
    ...
    protected Student find(String name, Node n)
    {
        ...
        //to get the name of student in current node
        String currentName = n.getNodeObject().getFullName();

        //found the student
        if (name.equalsIgnoreCase(currentName))
        {
            return n.getNodeObject(); //returns the student object
        }
        //recurse left
        if (name.compareToIgnoreCase(currentName)<0)
        {
    ...
    /**
     * This method is used to perform standard order walks over the tree.
     *
     * @param n The current node
     * @param sb The StringBuffer
     * @param type The type of walk : "preOrder", "inOrder", "postOrder"
     */
    protected void orderWalking(Node n, StringBuffer sb, String type)
    {
        if (n == null)
        ...
        if ("inOrder".equalsIgnoreCase(type))
        {
            //recurse left
            if (n.hasLeft())
            {
                orderWalking(n.left(), sb, type);
            }
            //visit node
            sb.append(n.getNodeObject().getFullName());
            sb.append(", ");
            //recurse right
            if (n.hasRight())
            {
                orderWalking(n.right(), sb, type);
            }
        }
    }

    /**
     * This method is used to perform a custom walk over the tree.
     *
     * @param n The current node
     * @param sb The StringBuffer
     */
    protected void ownWalking(Node n, StringBuffer sb)
    {
        ...
        if (n.hasLeft())
        {
            ownWalking(n.left(), sb);
        }
        // recurse right
        if (n.hasRight())
        {
            ownWalking(n.right(), sb);
        }
        // appends the last name
        sb.append(n.getNodeObject().getFullName());
        sb.append(", ");
